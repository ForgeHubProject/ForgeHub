...
/** Best-effort `lastUsedAt`/`lastUsedIp` bump for a user SSH key on successful git op. */
export function touchSshKey(sshKeyId: string, ip?: string): void {
  prisma.sSHKey.update({ where: { id: sshKeyId }, data: { lastUsedAt: new Date(), ...(ip ? { lastUsedIp: ip } : {}) } }).catch(() => {});
}

/** Best-effort `lastUsedAt`/`lastUsedIp` bump for a deploy key on successful git op. */
export function touchDeployKey(deployKeyId: string, ip?: string): void {
  prisma.deployKey.update({ where: { id: deployKeyId }, data: { lastUsedAt: new Date(), ...(ip ? { lastUsedIp: ip } : {}) } }).catch(() => {});
}
