 ...
 */
function ProfileRoute({ token, user, onLogout }: { token: string; user: User; onLogout: () => void }) {
  ...
  return <UserProfilePage token={token} user={user} onLogout={onLogout} />;
}
  ...
  return (
    <>
      ...
      <Route
        ...
        element={
          authed ? (
            <ProfileRoute token={token!} user={user!} onLogout={handleLogout} />
          ) : (
            <Navigate to="/login" replace />
          )
        }
